use crate::{AppWindow, fileReader, play, volume};  
use log::debug;
...
impl UICommandsReceiver{
    ...
    fn load_file(&mut self) {
        ...
        let file_reader = fileReader::FileReader::new(trimmed_file);
        ...
        let vol = Arc::new(Mutex::new(0.5));
        self.volume_handle = Some(Arc::clone(&vol));
        let volume = volume::Volume::new(file_reader.expect("..."),vol);
        self.file_player = Some(play::Play::new(volume));
